...
function ConsultationModel({phone, patientId}) {
  ...
  const headerElement = (
    <div>
      ...
      <div className="flex flex-row justify-content-between">
        ...
        <div className="mt-4">
         <WhatsappContact number={phone} message="Call or message patient"/>
        </div>
      </div>
      <div>
        <small>
          To consult a patient make a voice or video call via WhatsApp
        </small>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <Button label="Consult Patient" severity="success" onClick={() => setVisible(true)} />
      <Dialog header={headerElement} visible={visible} style={{ width: '90vw' }} onHide={() => {if (!visible) return; setVisible(false); }}>
        <div className="my-6">
          <FloatLabel>
            ...
            <label htmlFor="patient-complian">Patient's complain</label>
          </FloatLabel>
        </div>
        <div>
          <FloatLabel>
